...
if ( ! class_exists( __NAMESPACE__ . '\Kernel_Lite' ) ) {
	...
	abstract class Kernel_Lite {
		 ...
		 * @param string $handle Its handler.
		 *
		 * @return bool
		 */
		public function wp_admin_check_default_css_script( $src, $handle = '' ) {
		...
		public function lite_admin_script() {
			...
			foreach ( $wp_scripts->queue as $script ) {
				if ( ! in_array( $script, self::$appsbd_global_js ) ) {
					if ( ! $this->wp_admin_check_default_css_script( $wp_scripts->registered[ $script ]->src, $script ) ) {
		...
		public function lite_admin_style() {
			...
			foreach ( $wp_styles->queue as $style ) {
				if ( ! in_array( $style, self::$appsbd_global_css ) ) {
					if ( ! $this->wp_admin_check_default_css_script( $wp_styles->registered[ $style ]->src, $style ) ) {
