...
class POS_Product {
	...
	public static function get_product_from_woo_products_without_variables( $page = 1, $limit = 10, $src_props = array(), $orders = array() ) {
		...
		$post_type             = array( 'product', 'product_variation' );
		$post_status           = POS_Settings::get_module_instance()->get_product_status();
		$product_query         = \VitePos\Libs\POS_Product_Query::get_products( $page, $limit, $src_props, $orders, $post_type, $post_status, true );
		$product_obj           = new \stdClass();
		$product_obj->products = array();
		$product_obj->records  = $product_query->found_posts;
		foreach ( $product_query->posts as $product_id ) {
	...
	public static function get_price_html( $product ) {
		if($product->get_type()=='variable'){
			...
			if ( '' === $product->get_price() ) {
				...
				$price = self::format_sale_price( wc_get_price_to_display( $product,
	...
	public static function get_product_data( $product, $add_ctg_id = false, $is_no_variable = false ) {
		...
		if ( $product->meta_exists( '_vt_is_hidden' ) ) {
			$pos_product->is_hidden = $product->get_meta( '_vt_is_hidden' );
		} else {
			$pos_product->is_hidden = 'N';
		}
		/**
	...
	public static function get_product_variation_data( $product ) {
		...
		if ( POS_Settings::is_default_stock() ) {
			 ...
			 * @since 1.0.0
			 */
			$pos_product->stock_quantity = apply_filters( 'vitepos/filter/outlet-stock', 0, $pos_product->id, $pos_product->outlet_id );
		}
	...
	public static function update_product_stock( &$product, $outlet_id, $user_id, $quantity, $is_edit = false ) {
		...
		if ( POS_Settings::is_default_stock() ) {
			...
			if ( $pre_stock != $quantity ) {
				if ( $pre_stock > $quantity ) {
					return Mapbd_Pos_Stock_Log::AddLog( 'O', 0, $product->get_id(), $pre_stock, ( $pre_stock - $quantity ), $msg, $user_id, $ref_type, 'W' );
				} else {
					return Mapbd_Pos_Stock_Log::AddLog(
						'I',
						0,
						$product->get_id(),
						$pre_stock,
