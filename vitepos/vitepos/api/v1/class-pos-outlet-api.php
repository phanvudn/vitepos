...
class Pos_Outlet_Api extends API_Base {
	...
	public function withdraw_from_drawer() {
		...
		$user_note = $this->get_payload( 'user_note', '' );
		if ( empty( $outlet ) || empty( $counter ) ) {
			$this->add_error( 'Requested outlet or counter is empty' );
		...
		if ( ! Mapbd_Pos_Cash_Drawer::add_cash_entry( 'C', 'Cash Withdrawn', $amount, $cash_drawer, $this->get_current_user_id(), 'W', $user_note ) ) {
	...
	public function get_drawer_info( $cash_drawer ) {
		...
		$counters                        = Mapbd_Pos_Counter::fetch_all( 'id,name' );
			...
			$drawer_list = $drawer->select_all_grid_data( '', 'opening_time', 'DESC' );
			$drawer_data = array();
